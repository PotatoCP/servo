...
impl Handler {
    ...
    fn dispatch_actions_inner(&self, actions_by_tick: ActionsByTick) -> Result<(), ErrorStatus> {
        ...
        for tick_actions in actions_by_tick.iter() {
            ...
            self.wait_for_user_agent_handling_complete(cnt)?;
        }
            ...
            match self.constellation_receiver.recv() {
                Ok(response) => {
                    let current_waiting_id = self
                        .current_action_id
                        .get()
                        .expect("Current id should be set before dispath_actions_inner is called");
        ...
        for (input_id, action) in tick_actions.iter() {
            ...
            cnt += match action {
                ...
                ActionItem::Key(KeyActionItem::Key(KeyAction::Down(keydown_action))) => {
                    let res = self.dispatch_keydown_action(input_id, keydown_action);
                ...
                ActionItem::Pointer(PointerActionItem::Pointer(PointerAction::Down(
                    ...
                    let res = self.dispatch_pointerdown_action(input_id, pointer_down_action);
    ...
    fn dispatch_general_action(&self, source_id: &str) -> i32 {
        self.session()
            .unwrap()
            .input_state_table
            .borrow_mut()
    ...
    fn dispatch_keydown_action(&self, source_id: &str, action: &KeyDownAction) -> i32 {
    ...
    fn dispatch_keyup_action(&self, source_id: &str, action: &KeyUpAction) -> i32 {
        let session = self.session().unwrap();
    ...
    pub(crate) fn dispatch_pointerdown_action(&self, source_id: &str, action: &PointerDownAction) -> i32 {
        let session = self.session().unwrap();
    ...
    pub(crate) fn dispatch_pointerup_action(&self, source_id: &str, action: &PointerUpAction) -> i32 {
        let session = self.session().unwrap();
